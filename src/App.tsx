  ...
  if (isMobile) {
    return (
      <div className="min-h-screen bg-fog text-sand text-lg">
        {/* スマホ用ヘッダー */}
        <header className="sticky top-0 z-20 border-b border-white/10 bg-fog/95 px-6 py-5 backdrop-blur-lg">
          <div className="flex items-center justify-between">
            ...
            {session && (
              <button
                onClick={handleLock}
                className="rounded-full border border-white/20 bg-white/10 px-6 py-4 text-2xl font-semibold text-white"
              >
                🔒
              </button>
            )}
          ...
          <div className="flex min-h-[80vh] flex-col items-center justify-center px-8">
            <div className="w-full rounded-3xl border border-white/10 bg-white/5 p-10">
              ...
              <div className="mt-10 flex flex-wrap justify-center gap-4">
                <span className="rounded-full bg-white/10 px-5 py-3 text-lg text-slate-200">🔒 暗号化</span>
                <span className="rounded-full bg-white/10 px-5 py-3 text-lg text-slate-200">📴 オフライン</span>
              </div>
              <button
                onClick={handleReset}
                className="mt-8 w-full text-center text-xl text-slate-500 underline"
              >
                データを初期化
              </button>
            </div>
          ...
          <div className="pb-40">
            ...
            <div className="mt-6 grid grid-cols-2 gap-5 px-6">
              <div className="rounded-3xl border border-white/10 bg-white/10 p-6">
                ...
                <p className="mt-3 text-4xl font-bold text-mint">{formatCurrency(monthlySpent ?? 0)}</p>
              </div>
              <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
                <p className="text-lg text-slate-400">累計</p>
                <p className="mt-3 text-4xl font-bold text-white">{formatCurrency(totalSpent ?? 0)}</p>
              </div>
            </div>
            ...
            <div className="mt-6 space-y-6 px-6">
              <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
                ...
                <div className="space-y-5">
                  <input
                    className="w-full rounded-2xl border border-white/10 bg-white/5 px-6 py-6 text-2xl text-white placeholder-slate-500"
                  ...
                  <div className="flex gap-4">
                    <input
                      type="date"
                      className="flex-1 rounded-2xl border border-white/10 bg-white/5 px-5 py-6 text-2xl text-white"
                      value={draft.visitedAt}
                      onChange={(e) => setDraft((prev) => ({ ...prev, visitedAt: e.target.value }))}
                    />
                    <input
                      inputMode="numeric"
                      className="flex-1 rounded-2xl border-2 border-mint/50 bg-mint/10 px-5 py-6 text-3xl font-bold text-mint placeholder-mint/50"
                      value={draft.total}
                  ...
                  <select
                    className="w-full rounded-2xl border border-white/10 bg-white/5 px-6 py-6 text-2xl text-white"
              ...
              {/* 画像保存オプション */}
              <label className="flex items-center gap-5 px-3 text-xl text-slate-300">
                <input
                  type="checkbox"
                  checked={saveImage}
            ...
            <div className="mt-8 px-6">
              ...
              <div className="mt-5 space-y-5">
                {session.vault.receipts.length === 0 ? (
                  <p className="rounded-3xl bg-white/5 py-12 text-center text-xl text-slate-400">
                    まだレシートがありません
                  </p>
                ) : (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-2xl font-semibold text-white">{receipt.storeName}</p>
                          <span className="mt-2 inline-block rounded-full bg-white/10 px-4 py-2 text-lg text-slate-300">
                            {receipt.category}
                          </span>
                        </div>
                        <div className="text-right">
                          ...
                          <button
                            ...
                            className="mt-3 text-xl text-red-400"
                          >
                            削除
                          </button>
                        </div>
                ...
                {filteredReceipts.length > visibleCount && (
                  <button
                    ...
                    className="w-full rounded-2xl border border-white/10 bg-white/5 py-5 text-xl font-semibold text-white"
                  >
                    もっと見る
                  </button>
                )}
            ...
            <div className="mt-8 px-6 pb-6">
              <div className="flex gap-5">
                <button
                  ...
                  className="flex-1 rounded-2xl border border-white/15 bg-white/10 py-5 text-xl font-semibold text-white"
                >
                  📤 CSV保存
                </button>
                <label className="flex flex-1 cursor-pointer items-center justify-center rounded-2xl border border-white/15 bg-white/10 py-5 text-xl font-semibold text-white">
                  📥 CSV読込
                  <input
                    type="file"
                    accept=".csv,text/csv"
        ...
        {/* スマホ用固定フッター */}
        {session && (
          <div className="fixed inset-x-0 bottom-0 z-30 border-t border-white/10 bg-fog/95 px-6 py-6 backdrop-blur-lg">
            <div className="flex items-center gap-5">
              <button
                onClick={cameraActive ? stopCamera : startCamera}
                className={clsx(
                  "flex-1 rounded-2xl py-7 text-2xl font-bold",
                  cameraActive
              ...
              <button
                ...
                className="flex-[1.5] rounded-2xl border-2 border-mint bg-mint py-7 text-3xl font-bold text-fog shadow-lg disabled:opacity-50"
              >
                📸 撮影
              </button>
              <button
                onClick={handleSaveReceipt}
                className="flex-1 rounded-2xl border border-white/20 bg-white/15 py-7 text-2xl font-bold text-white"
              >
                💾 保存
              </button>
            </div>
  ...
  if (isMobile) {
    ...
    return (
      <div className="flex flex-col gap-6">
        <label className="text-2xl text-slate-200">
          🔑 パスフレーズ
          <input
            type="password"
            className="mt-5 w-full rounded-2xl border border-white/10 bg-white/5 px-6 py-7 text-3xl text-white outline-none ring-mint/30 focus:ring-2"
        ...
        <button
          ...
          className="w-full rounded-2xl bg-gradient-to-r from-mint/70 to-mint px-6 py-8 text-3xl font-bold text-fog shadow-soft transition hover:translate-y-[-1px] disabled:cursor-not-allowed disabled:opacity-60"
        >
